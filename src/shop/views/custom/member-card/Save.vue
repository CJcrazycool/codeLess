			...
			<div class="form-area">
				...
				<el-row>
				  <el-form-item label="权益说明">
					<el-input v-model="form.description" placeholder="请填写权益说明" type="textarea">
					</el-input>
				  </el-form-item>
				</el-row>
				<el-row>
				  <el-form-item label="充值说明">
					<el-input v-model="form.rechargeDescription" placeholder="请填写充值说明" type="textarea">
					</el-input>
				  </el-form-item>
				</el-row>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			load(id){
				this.$request.load(memberApi.getMemberCardType({
					params: {
						id
					},
					before: () => {
			...
			save(){
				...
				this.$request.submit(memberApi.editMbrCardType({
					data: this.form,
					before: () => {
